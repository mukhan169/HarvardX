...
data <- read.csv("https://https://raw.githubusercontent.com/mukhan169/ChooseYourOwn/master/Data/data.csv")
...
# General Data Info
#Lets find out how many rows and Columns we have,
paste('The breast cancer dataset has',nrow(data),'rows and',ncol(data),'columns.')
str(data)
summary(data)
...
ggplot(data, aes(x=diagnosis))+geom_bar(fill="red",alpha=0.5)+theme_bw()+labs(title="Distribution of Diagnosis")

# Check to see the features looks for different diagnosis
data_gather <- data[, -1] %>%
